...
# and a standard deviation of 30cc. What is a 95% Student's T confidence interval for the 
...
# To test the systems, administrators selected 20 nights and randomly assigned 
# the new triage system to be used on 10 nights and the standard system on the remaining 10 nights. 
...
# mean MWT associated with the new system. Assume a constant variance. 
...
s_pool=sqrt(((n - 1) * old_var + (n - 1) * new_var)/(n + n - 2))

new_mwt - old_mwt + c(-1, 1) * qt(quant, df=n+n-2) * s_pool * (1 / n + 1 / n)^.5
...
# to see a physician. The average MWT for the new system was 4 hours with a
# standard deviation of 0.5 hours while the average MWT for the old system was 6 hours with a
# standard deviation of 2 hours. Consider the hypothesis of a decrease in the mean MWT
# associated with the new treatment. What does the 95% independent group confidence interval
# with unequal variances suggest vis a vis this hypothesis?
...
# the four week period appear to differ between the treated and placebo groups? 
# Assuming normality of the underlying data and a common population variance, 
# calculate the relevant *90%* t confidence interval. Subtract in the order of (Treated - Placebo)
